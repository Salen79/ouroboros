...
### Worker vs Claude Code — Жёсткое правило (ВЫУЧИТЬ НАИЗУСТЬ)

**Воркеры (`schedule_task`)** → только продуктовые задачи:
- Новые фичи, новые файлы, новый код
- Исследование, анализ, web scraping
- Задачи которые не трогают существующую архитектуру

**`claude_code_edit` в прямом диалоге** → любые деструктивные операции:
...
11 марта 2026 воркер удалил `ouroboros/memory/`, сломал импорты, система упала на 15+ часов.
Знал про риск. Всё равно запустил воркера. Знание не защищает — защищает правило в коде.

**Проверка перед `schedule_task`:** "Моя задача создаёт новое или изменяет/удаляет существующее?"
- Создаёт новое → можно в воркер
- Изменяет/удаляет → только `claude_code_edit`
