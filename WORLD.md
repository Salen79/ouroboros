...
- В Colab неизменяемой должна быть только минимальная boot-ячейка (shim): git sync + запуск `colab_launcher.py` из репозитория.
- Полная супервизорная логика живёт в `colab_launcher.py` и может эволюционировать через обычные git-коммиты в `ouroboros`.
- `/panic` и `/restart` обрабатываются супервизором (не LLM); это supervisor-owned контур, а не поведение LLM.
...
## Протокол конфигурации и секретов

- В Colab Secrets хранятся только ключи и бюджет: `OPENROUTER_API_KEY`, `TELEGRAM_BOT_TOKEN`, `TOTAL_BUDGET`, `GITHUB_TOKEN` (+ опционально `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`).
- Несекретные параметры (`GITHUB_USER`, `GITHUB_REPO`, `OUROBOROS_*`) задаются через env/config-ячейку или берутся из дефолтов кода.
- Legacy fallback из Secrets для несекретных параметров допускается только как временная миграция.

## Протокол зависимостей

- Источник зависимостей — `requirements.txt`.
- При bootstrap/restart/branch-switch launcher переустанавливает зависимости из `requirements.txt`.
- Fallback на минимальный набор (`openai>=1.0.0`, `requests`) допустим только если `requirements.txt` отсутствует.
...
- Перед любым `git reset --hard` супервизор проверяет unsynced-state; для owner/agent restart reset блокируется при dirty/unpushed.
- При блокировке reset автоматически создаётся rescue-снимок (`MyDrive/Ouroboros/archive/rescue/...`) с diff и untracked файлами.
