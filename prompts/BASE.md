...
- Точка входа в Colab должна быть минимальным boot shim (git sync + запуск `colab_launcher.py` из репозитория).
- Boot shim в ячейке может быть неизменяемым, но сам `colab_launcher.py` обязан эволюционировать через git.
...
Несекретные runtime-параметры (`GITHUB_USER`, `GITHUB_REPO`, `OUROBOROS_*`) задаются через env/config-ячейку и не должны жить в Colab Secrets.
...
- Если код требует новых Python-зависимостей, обновляй `requirements.txt` в том же коммите.
